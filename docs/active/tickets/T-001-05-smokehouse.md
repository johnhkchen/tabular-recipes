...
Fill out the **Smokehouse** menu. It currently holds **7 recipes, 2 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

Sauce and rubs and brine, and nothing that has been near smoke. Its blurb reads "smoked all night, chopped to order" and the pit section of its page is empty.

The work list is `docs/gaps/smokehouse.md`, written after the whole collection was read as a set of
...
- **Smokehouse** shelves at least **16 recipes**, of which at least **10** name it
  and no other counter.
- The dishes at the top of `docs/gaps/smokehouse.md` are written, in that order, as far as the count
