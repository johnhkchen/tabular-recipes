...
Fill out the **Curry House** menu. It currently holds **15 recipes, 15 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

The breads are here and the sauce list is not. A curry-house board prints a list of SAUCES run across every protein, so korma and bhuna are rows and chicken and lamb are columns; ten printed lines rest on one onion-tomato masala base that does not exist yet.

The work list is `docs/gaps/curry-house.md`, written after the whole collection was read as a set of
...
- **Curry House** shelves at least **22 recipes**, of which at least **20** name it
  and no other counter.
- The dishes at the top of `docs/gaps/curry-house.md` are written, in that order, as far as the count
