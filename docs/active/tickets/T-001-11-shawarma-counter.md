...
Fill out the **Shawarma Counter** menu. It currently holds **21 recipes, 15 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

The dip case is the strongest thing on the site and the spit is empty. Nothing here is the shaved meat the counter is named for, and there is no falafel, because nothing in the collection is deep-fried.

The work list is `docs/gaps/shawarma-counter.md`, written after the whole collection was read as a set of
...
- **Shawarma Counter** shelves at least **26 recipes**, of which at least **18** name it
  and no other counter.
- The dishes at the top of `docs/gaps/shawarma-counter.md` are written, in that order, as far as the count
