...
Fill out the **Dim Sum Counter** menu. It currently holds **9 recipes, 3 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

The steamer baskets are empty. Pieces are marked "(3)" to an order on these boards, and the har gow / siu mai / char siu bao core is what a person pictures when they hear the words.

The work list is `docs/gaps/dim-sum-counter.md`, written after the whole collection was read as a set of
...
- **Dim Sum Counter** shelves at least **18 recipes**, of which at least **12** name it
  and no other counter.
- The dishes at the top of `docs/gaps/dim-sum-counter.md` are written, in that order, as far as the count
