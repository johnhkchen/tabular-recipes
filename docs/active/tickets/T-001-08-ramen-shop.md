...
Fill out the **Ramen Shop** menu. It currently holds **10 recipes, 9 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

The top-level split on a ramen board is the BROTH, not the protein — tonkotsu gets its own section with six variations inside it — and no broth is written. Toppings are sold in bundled sets rather than singly.

The work list is `docs/gaps/ramen-shop.md`, written after the whole collection was read as a set of
...
- **Ramen Shop** shelves at least **18 recipes**, of which at least **14** name it
  and no other counter.
- The dishes at the top of `docs/gaps/ramen-shop.md` are written, in that order, as far as the count
