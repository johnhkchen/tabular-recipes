...
Fill out the **Thai Kitchen** menu. It currently holds **5 recipes, 5 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

Five recipes, and the reference says a Thai board leads with curries listed by colour and a lunch section where every line is prefixed L-. The pastes are here; the curries they go into are not.

The work list is `docs/gaps/thai-kitchen.md`, written after the whole collection was read as a set of
...
- **Thai Kitchen** shelves at least **16 recipes**, of which at least **14** name it
  and no other counter.
- The dishes at the top of `docs/gaps/thai-kitchen.md` are written, in that order, as far as the count
