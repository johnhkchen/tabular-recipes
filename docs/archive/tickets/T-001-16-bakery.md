...
Fill out the **Bakery** menu. It currently holds **93 recipes, 58 of them its own** —
the rest are borrowed from other counters, which is legitimate but does not make a menu.

The fullest counter on the site, and still missing the laminated doughs and the enriched morning things that a bakery case is built around. Lowest priority: this one is topping up, not filling.

The work list is `docs/gaps/bakery.md`, written after the whole collection was read as a set of
...
- **Bakery** shelves at least **97 recipes**, of which at least **62** name it
